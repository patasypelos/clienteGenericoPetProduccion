...
function ReporteVentas() {
  ...
  const handleFilterMascota = () => {
    debugger;
    let url = `${baseUrl}/RegistrarUsuariosMascotas/GetConsultarMascotas?fechaInici=${fechaInicio}&fechaFin=${fechaFin}&idArticulo=${articuloSeleccionado}`;
    ...
    .then(data => {
      setData(data);
          ...
          .filter(venta => venta.nombreTipoArticulo === 'BAÑO GENERAL')
    ...
    <option key={option.value} value={option.value}>
      {option.text}
            ...
            <Grid item xs={12}>
              ...
              variant="contained" onClick={handleFilterMascota}>
                Filtrar
              </Button>
            </Grid>
          </Grid>
        ...
        <SoftBox>
        <Grid  item xs={12} md={4}>
          Total ganacia: {totalPrecioPurina.toFixed(0)}
        </Grid> 
